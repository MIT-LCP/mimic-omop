...
"admissions" as (SELECT subject_id as hadm_subject_id, hadm_id, mimic_id as visit_occurrence_id, diagnosis, coalesce(edregtime, admittime) as condition_start_datetime, dischtime as condition_end_datetime FROM mimic.admissions),
"patients" as (SELECT subject_id, mimic_id as person_id FROM mimic.patients),
"adm_diag_cpt" AS (SELECT * FROM mimic.gcpt_admissions_diagnosis_to_concept)
...
UNION ALL
SELECT
  nextval('mimic.mimic_id_seq') AS condition_occurrence_id       
