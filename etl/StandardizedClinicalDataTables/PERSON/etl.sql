...
admissions AS (SELECT DISTINCT ON (subject_id) subject_id as person_id, ethnicity as ethnicity_source_value FROM mimic.admissions ORDER BY subject_id, admittime DESC) 
...
FROM patients
LEFT JOIN admissions USING (person_id) ;
