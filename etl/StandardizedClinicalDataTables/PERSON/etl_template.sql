...
gcpt_ethnicity_to_concept AS (SELECT race_concept_id as race_concept_id, ethnicity_concept_id as ethnicity_concept_id FROM mimic.gcpt_ethnicity_to_concept),
admissions AS (SELECT ethnicity as ethnicity_source_value FROM mimic.admissions) 
 ...
 SELECT patients.person_id, patients.gender_concept_id, patients.year_of_birth, patients.month_of_birth, patients.day_of_birth, patients.birth_datetime, gcpt_ethnicity_to_concept.race_concept_id, gcpt_ethnicity_to_concept.ethnicity_concept_id, patients.gender_source_value, admissions.ethnicity_source_value 
FROM patients
 LEFT JOIN gcpt_ethnicity_to_concept ON ()
