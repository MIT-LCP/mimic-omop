 ...
 WITH tmp as (SELECT *, CASE WHEN prev_wardid = curr_wardid THEN curr_wardid ELSE curr_wardid END as value, curr_wardid, prev_wardid FROM mimic.transfers),
transfers_no_bed as(SELECT distinct on (hadm_id, value) tmp.*, min(intime) OVER(PARTITION BY hadm_id, value) as intime_real, max(outtime) OVER(PARTITION BY hadm_id, value) as outtime_real FROM tmp WHERE curr_careunit IS NOT NULL),-- curr careunit is null for a row dedicated for discharge, does not contain meaningful informations
transfers AS (SELECT subject_id, hadm_id, curr_careunit, mimic_id as visit_detail_id, 9201 as visit_detail_concept_id, intime::date as visit_start_date, intime as visit_start_datetime, outtime::date as visit_end_date, outtime as visit_end_datetime, 44818518 as visit_type_concept_id, eventtype as discharge_to_source_value, LAG(mimic_id) OVER (PARTITION BY hadm_id ORDER BY intime ASC) as preceding_visit_detail_id FROM transfers_no_bed),
