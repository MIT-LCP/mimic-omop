...
WITH 
"transfers_bed" as (SELECT *, CASE WHEN prev_wardid = curr_wardid THEN curr_wardid ELSE curr_wardid END as value, curr_wardid, prev_wardid FROM mimic.transfers WHERE outtime IS NOT NULL), 
...
"transfers_call" AS (SELECT transfers_bed.*, CASE WHEN transfers_bed.icustay_id IS NULL THEN NULL ELSE discharge_delay END AS discharge_delay FROM transfers_bed LEFT JOIN callout_delay ON (transfers_bed.hadm_id = callout_delay.hadm_id AND mean_time between intime and outtime AND callout_delay.curr_careunit = transfers_bed.curr_careunit)),
...
"transfers" AS (SELECT subject_id, hadm_id, curr_careunit, mimic_id as visit_detail_id, 9201 as visit_detail_concept_id, transfers_no_bed.intime_real::date as visit_start_date, intime_real as visit_start_datetime, outtime_real::date as visit_end_date, outtime_real as visit_end_datetime, 44818518 as visit_type_concept_id, eventtype as discharge_to_source_value, LAG(mimic_id) OVER (PARTITION BY hadm_id ORDER BY transfers_no_bed.intime_real ASC) as preceding_visit_detail_id, discharge_delay FROM transfers_no_bed),
...
"gcpt_admission_location_to_concept" AS (SELECT concept_id as admitting_source_concept_id, admission_location FROM mimic.gcpt_admission_location_to_concept),
