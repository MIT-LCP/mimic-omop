...
-- ;
------------------------------------------------------------------------------------------------------------------------------------
 \copy (SELECT measurement_id, person_id , measurement_concept_id , replace(  measurement_concept_name, E'\\', '\\\\' ) as measurement_concept_name , replace(  measurement_concept_code, E'\\', '\\\\' ) as measurement_concept_code , replace(  measurement_concept_code_system, E'\\', '\\\\' ) as measurement_concept_code_system , measurement_date , measurement_datetime , measurement_type_concept_id , replace(  measurement_type_concept_name, E'\\', '\\\\' ) as measurement_type_concept_name , operator_concept_id , replace(  operator_concept_name, E'\\', '\\\\' ) as operator_concept_name , value_as_number , value_as_concept_id , unit_concept_id , replace(  unit_concept_name, E'\\', '\\\\' ) as unit_concept_name , replace(  unit_concept_code, E'\\', '\\\\' ) as unit_concept_code , replace(  unit_concept_code_system, E'\\', '\\\\' ) as unit_concept_code_system , range_low , range_high , provider_id , visit_occurrence_id , visit_detail_id , replace( measurement_source_value, E'\\', '\\\\' ) as measurement_source_value , measurement_source_concept_id , replace( unit_source_value, E'\\', '\\\\' ) as unit_source_value , replace( value_source_value, E'\\', '\\\\' ) as value_source_value FROM omop.measurement_denorm) TO PROGRAM 'gzip > etl/Result/measurement.csv.gz' CSV DELIMITER '|' HEADER QUOTE '"';
 \copy (SELECT * FROM omop.condition_occurrence_denorm) TO PROGRAM 'gzip > etl/Result/condition_occurrence.csv.gz' CSV DELIMITER '|' HEADER QUOTE '"';
 \copy (SELECT * FROM omop.person_denorm) TO PROGRAM 'gzip > etl/Result/person.csv.gz' CSV DELIMITER '|' HEADER QUOTE '"';
 \copy (SELECT * FROM omop.visit_occurrence_denorm) TO PROGRAM 'gzip > etl/Result/visit_occurrence.csv.gz' CSV DELIMITER '|' HEADER QUOTE '"';
 \copy (SELECT * FROM omop.visit_detail_denorm) TO PROGRAM 'gzip > etl/Result/visit_detail.csv.gz' CSV DELIMITER '|' HEADER QUOTE '"';
 \copy (SELECT * FROM omop.care_site_denorm) TO PROGRAM 'gzip > etl/Result/care_site.csv.gz' CSV DELIMITER '|' HEADER QUOTE '"';
 ...
 \copy (SELECT * FROM omop.concept_relationship) TO PROGRAM 'gzip > etl/Result/concept_relationship.csv.gz' CSV DELIMITER '|' HEADER QUOTE '"'; 
 \copy (SELECT * FROM omop.fact_relationship) TO PROGRAM 'gzip > etl/Result/fact_relationship.csv.gz' CSV DELIMITER '|' HEADER QUOTE '"';
